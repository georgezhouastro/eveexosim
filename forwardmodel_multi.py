...
def draw_planet(baseline, master_raw, synthetic_raw, interpolator_pack, useUV=True, useIR=True, fov=5, psf=5, custom_ra=None, custom_dec=None, snr_optical_scalar=1.0, snr_ir_scalar=1.0):
    ...
    all_simulated_planets = []

    # Pre-extract arrays for fast filtering of similar stars for multi-planets
    mstar_array = df['mstar'].values
    age_array = df['age'].values
    per_array = df['per'].values
    prad_array = df['prad'].values

    for i in range(len(df)):
        row_dict = df.iloc[i].to_dict() # Retain all original stellar data
        
        teff = row_dict['teff']
        mstar = row_dict['mstar']
        rstar = row_dict['rstar']
        tmag = row_dict['tmag']
        age = row_dict['age']
        obs_fields = row_dict['observed_fields']
        nobs = row_dict['nobs']

        # ---------------------------------------------------------
        # 1. Calculate stellar noise and SNR (Once per star)
        # ---------------------------------------------------------
        ...
        if snr_optical > 297.7: snr_optical = 297.7
        if snr_optical < 0.1: snr_optical = 0.1 

        snr_IR = eve_ir[0] * TESS_flux / np.sqrt(eve_ir[1]*TESS_flux + eve_ir[2])
        snr_IR *= snr_ir_scalar
        if snr_IR < 0.1: snr_IR = 0.1
        ...
        # ---------------------------------------------------------
        n_planets = np.random.poisson(lam=0.689)

        # Draw cosi, fixed across all planets in the system.
        cosi = np.random.uniform(low=0, high=1)

        # If 0 planets, append empty record so star isn't lost from catalog
        if n_planets == 0:
            empty_record = row_dict.copy()
            empty_record.update({
                'pl_period': np.nan, 'pl_radius': np.nan, 'delta': np.nan,
                'sigmaOptical': sigma_optical, 'sigmaIR': sigma_IR,
                'npoints': np.nan, 'ntransits': np.nan, 'draw': np.nan, 'T_retrieve_final': 0
            })
            for v in range(1, max_visits + 1):
                empty_record[f'snr_v{v}'] = np.nan
                empty_record[f'T_retrieve_v{v}'] = 0
            all_simulated_planets.append(empty_record)
            continue

        # ---------------------------------------------------------
        # 3. Simulate Each Planet in the System
        # ---------------------------------------------------------
        for p_idx in range(n_planets):
            planet_record = row_dict.copy()
            
            # Setup base observables structure
            planet_record.update({
                'pl_period': np.nan, 'pl_radius': np.nan, 'delta': np.nan,
                'sigmaOptical': sigma_optical, 'sigmaIR': sigma_IR,
                'npoints': np.nan, 'ntransits': np.nan, 'draw': np.nan, 'T_retrieve_final': 0
            })
            for v in range(1, max_visits + 1):
                planet_record[f'snr_v{v}'] = np.nan
                planet_record[f'T_retrieve_v{v}'] = 0
            ...
            if p_idx == 0:
                ...
                # Find stars within 10% M* and Age
                mask = (mstar_array >= 0.9 * mstar) & (mstar_array <= 1.1 * mstar) & \
                       (age_array >= 0.9 * age) & (age_array <= 1.1 * age)
                valid_indices = np.where(mask)[0]
                
                if len(valid_indices) > 0:
                    chosen_idx = np.random.choice(valid_indices)
                else:
                    # Fallback if no matching star exists in the sample bounds
                    chosen_idx = np.random.randint(len(df)) 
                    
                p = per_array[chosen_idx]
                rp = prad_array[chosen_idx]

            # Geometrical Transit Test using shared cosi
            a = (p * 24 * 3600 * np.sqrt(G_CONST * M_SUN * mstar) / (2 * np.pi))**(2/3.0)
            ars = a / (R_SUN * rstar)
            b = ars * cosi

            # If b < 1.0, the planet geometry results in a transit
            if b < 1.0:
                t0 = np.random.uniform(0, p)
                rec_draw = np.random.uniform(0, 1)
                
                planet_record['draw'] = rec_draw
                planet_record['pl_period'] = p
                planet_record['pl_radius'] = rp
                ...
                for v in range(1, nobs + 1):
                    ...
                    # (Assuming UVdetectable is a function accepting a dict or row)
                    if UVdetectable(planet_record) and useUV:
                    ...
                    planet_record[f'snr_v{v}'] = snr
                    planet_record[f'T_retrieve_v{v}'] = is_retrieved

                    if v == 1:
                        planet_record['delta'] = delta

                planet_record['npoints'] = npoints
                planet_record['ntransits'] = ntransits
                planet_record['T_retrieve_final'] = planet_record[f'T_retrieve_v{nobs}']

            all_simulated_planets.append(planet_record)

    # Convert the flattened list of all simulated planets into a DataFrame
    final_df = pd.DataFrame(all_simulated_planets)
    return final_df
...
def simulation_summary(filepath):
    ...
    unique_stars = df['tic'].nunique()
    multi_planet_systems = sum(df['tic'].value_counts() > 1)

    print("--- All Planets of all ages ---")
    print(f"Total retrieved planets: {total_planets}")
    ...
    print(f"Super Earths: {sum(SE)}, Sub-Neptunes: {sum(SN)}, Giants: {sum(G)}\n")
    
    df_young = df[df['age'] < 50]
    total_young = len(df_young)
    unique_young_stars = df_young['tic'].nunique()
    multi_young = sum(df_young['tic'].value_counts() > 1)

    print("--- (<50 Myr) ---")
    print(f"Total retrieved planets: {total_young}")
    print(f"Unique host stars: {unique_young_stars}")
    print(f"Multi-planet systems: {multi_young}")

    SE_y = (df_young['pl_radius'] > 1.25) & (df_young['pl_radius'] < 2)
    SN_y = (df_young['pl_radius'] > 2) & (df_young['pl_radius'] < 4)
    G_y = df_young['pl_radius'] > 4        

    print(f"Super Earths: {sum(SE_y)}, Sub-Neptunes: {sum(SN_y)}, Giants: {sum(G_y)}\n")
...
def field_yield_summary(filepath, fov=5.0, custom_ra=None, custom_dec=None):
    ...
    print("--- Young Planet Yield (<50 Myr) by Target Field ---")
    
    seen_visits = {} # Tracks visits per STAR (tic)
    retrieved_planet_indices = set() # Tracks retrievals per PLANET (row index)
    ...
    for i, row in targetfields.iterrows():
        ...
        N_systems_in_field = planets_in_field['tic'].nunique()
        
        # 2. Increment the visit index for the unique stars in this field
        for tic in planets_in_field['tic'].unique():
            seen_visits[tic] = seen_visits.get(tic, 0) + 1
            
        new_planets_yield = 0
        new_systems_yield = set()
        
        for idx, planet in planets_in_field.iterrows():
            # If this specific planet crossed SNR threshold in an earlier field, skip counting
            if idx in retrieved_planet_indices:
                continue
            ...
            if col_name in planet and planet[col_name] == 1:
                retrieved_planet_indices.add(idx)
                new_planets_yield += 1
                new_systems_yield.add(tic)
        
        print(f"Field {i:02d} (RA: {ra_target:6.2f}, DEC: {dec_target:6.2f}) | "
              f"New Planets: {new_planets_yield:3d} (New Systems: {len(new_systems_yield):3d}) | "
              f"Total observable planets in field: {N_planets_in_field:4d} (Total Systems: {N_systems_in_field:4d})")
