...
def draw_outer_inclination(i1_deg,
                           alpha=1.5,
                           beta_param=5.0,
                           i_max_deg=5.0):
    """
    Draw an outer-planet line-of-sight inclination given an inner planet.

    Parameters
    ----------
    i1_deg : float or array_like
        Inner planet inclination in degrees.
    alpha, beta_param : float
        Shape parameters of the Beta distribution for mutual inclinations.
    i_max_deg : float
        Maximum mutual inclination (degrees) for the Beta distribution support.

    Returns
    -------
    i2_deg : float or ndarray
        Drawn outer planet inclination(s) in degrees.
    """
    # convert to radians
    i1 = np.radians(i1_deg)

    # draw mutual inclination ?i ? Beta(?,?) scaled to [0, i_max]
    delta_i = np.random.beta(alpha, beta_param, size=np.shape(i1)) \
              * np.radians(i_max_deg)

    # draw node angle ? uniformly [0, 2?)
    omega = np.random.uniform(0, 2*np.pi, size=np.shape(i1))

    # compute cos(i2) via spherical-law-of-cosines for orbital planes
    cos_i2 = (np.cos(i1)*np.cos(delta_i) +
              np.sin(i1)*np.sin(delta_i)*np.cos(omega))

    # guard against numerical round-off pushing outside [-1,1]
    cos_i2 = np.clip(cos_i2, -1.0, 1.0)

    # return i2 in degrees
    i2 = np.degrees(np.arccos(cos_i2))
    return i2
...
def draw_planet(baseline, master_raw, interpolator_pack, useUV=True, useIR=True, fov=5, psf=5, custom_ra=None, custom_dec=None, snr_optical_scalar=1.0, snr_ir_scalar=1.0):
    rms_dist, rms_dist_orion = interpolator_pack

    masterlist = master_raw.drop_duplicates('tic').copy()
    df, tic_catalog_list = prepareinputfields(masterlist, fov=fov, custom_ra=custom_ra, custom_dec=custom_dec)
    ...
    # Pre-allocate the base NaN structure to ensure consistent columns across the output
    base_nan_dict = {
        'pl_period': np.nan, 'pl_radius': np.nan, 'delta': np.nan,
        'sigmaOptical': np.nan, 'sigmaIR': np.nan, 'npoints': np.nan,
        'ntransits': np.nan, 'draw': np.nan, 'T_retrieve_final': 0
    }
    for v in range(1, max_visits + 1):
        base_nan_dict[f'snr_v{v}'] = np.nan
        base_nan_dict[f'T_retrieve_v{v}'] = 0

    output_records = []

    for i, row in enumerate(df.itertuples()):
        row_dict = row._asdict()
        row_dict.pop('Index', None)  # Remove the tuple index to avoid DataFrame column clutter
        ...
        snr_optical = np.clip(snr_optical * snr_optical_scalar, 0.1, 297.7)

        snr_IR = eve_ir[0] * TESS_flux / np.sqrt(eve_ir[1]*TESS_flux + eve_ir[2])
        snr_IR = max(snr_IR * snr_ir_scalar, 0.1)
        ...
        # On-the-fly planet generation per star
        if teff < 5300: star_type = 'K'
        elif 5300 <= teff < 6000: star_type = 'G'
        else: star_type = 'F'

        if teff > 7000:
            p, rp = [], []
        else:
            F_occ = sample_occ.Occ(star_type)
            F_occ.sample()
            F_occ.cal_cum()
            p, rp = F_occ.draw()

        if len(p) > 0:
            a_inner = (p[0] * 24 * 3600 * np.sqrt(G_CONST * M_SUN * mstar) / (2 * np.pi))**(2/3.0)
            ars_inner = a_inner / (R_SUN * rstar)
            trprob = 1 / ars_inner

            if np.random.uniform(0, 1) < trprob:
                # Inner planet transits
                b_inner = np.random.uniform(0, 1)
                inc_inner = np.arccos(b_inner / ars_inner)
                dilution = determinedilution(df, i, tic_catalog_list[obs_fields[-1]], psf=psf)

                # Track multiple planets in this specific system
                for pl in range(len(p)):
                    out_dict = row_dict.copy()
                    out_dict.update(base_nan_dict) # Base NaN padding

                    # Convert radius from Solar to Earth units 
                    rp_earth = rp[pl] * (R_SUN / R_EARTH)
                    
                    a_pl = (p[pl] * 24 * 3600 * np.sqrt(G_CONST * M_SUN * mstar) / (2 * np.pi))**(2/3.0)
                    ars_pl = a_pl / (R_SUN * rstar)

                    if pl == 0:
                        b_pl = np.cos(inc_inner) * ars_pl
                    else:
                        inc_outer_deg = draw_outer_inclination(inc_inner)
                        b_pl = ars_pl * np.cos(inc_outer_deg * np.pi / 180)

                    rec_draw = np.random.uniform(0, 1)
                    out_dict['draw'] = rec_draw
                    out_dict['pl_period'] = p[pl]
                    out_dict['pl_radius'] = rp_earth

                    if b_pl < 1:
                        # This specific planet transits
                        t0 = np.random.uniform(0, p[pl])

                        for v in range(1, nobs + 1):
                            effective_baseline = v * baseline
                            snr_opt, delta, _, npoints, ntransits = calc_transit_snr(
                                p[pl], mstar, rstar, rp_earth, b_pl, effective_baseline, sigma_optical, dilution
                            )
                            snr_ir, _, _, _, _ = calc_transit_snr(
                                p[pl], mstar, rstar, rp_earth, b_pl, effective_baseline, sigma_IR, dilution
                            )
                            ...
                            # Validation: Did it cross the minimum timeline threshold?
                            if is_retrieved and (t0 + p[pl] > effective_baseline):
                                is_retrieved = 0

                            out_dict[f'snr_v{v}'] = snr
                            out_dict[f'T_retrieve_v{v}'] = is_retrieved

                            if v == 1:
                                out_dict['delta'] = delta
                                out_dict['sigmaOptical'] = sigma_optical
                                out_dict['sigmaIR'] = sigma_IR

                        out_dict['npoints'] = npoints
                        out_dict['ntransits'] = ntransits
                        out_dict['T_retrieve_final'] = out_dict[f'T_retrieve_v{nobs}']

                    # Append planet (whether transiting or not) to system catalog
                    output_records.append(out_dict)
            else:
                # Inner planet doesn't transit; yield NaN system
                out_dict = row_dict.copy()
                out_dict.update(base_nan_dict)
                output_records.append(out_dict)
        else:
            # Star draws zero planets; yield NaN system
            out_dict = row_dict.copy()
            out_dict.update(base_nan_dict)
            output_records.append(out_dict)

    # Reconstruct single DataFrame at the very end
